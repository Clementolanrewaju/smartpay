...
class ServiceResponse<T> {
  ...
  AsyncValue<T> toAsyncValue() {
    return status ? AsyncData(data!) : AsyncError(message!);
  }
}
...
Future<ServiceResponse<T>> serveFuture<T>({
  ...
  try {
    ...
    String _error = handleError == null ? e.toString() : handleError(e);
    return serveError<T>(error: _error);
  }
}
...
Stream<ServiceResponse<T>> serveStream<T>({
  ...
  try {
    ...
    String _error = handleError == null ? e.toString() : handleError(e);
    yield* Stream.value(serveError<T>(error: _error));
  }
}
